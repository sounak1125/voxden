...
check('aspirated kh folds onto k', phon.phoneticCode('Sakhi'), phon.phoneticCode('sucky'));
check('aspirated bh folds onto b', phon.phoneticCode('bhai'), 'B');
check('v and w merge', phon.phoneticCode('Navya'), phon.phoneticCode('no way'));
check('leading vowel kept as one marker', phon.phoneticCode('Aishwarya'), 'ASVR');
...
const subh = phon.generateVariants('Subhrajit', 10);
...
const SAMPLE_NAMES = ['Subhrajit', 'Sounak', 'Sakhi', 'Navya', 'Aishwarya', 'Voxden',
  'Thakumar', 'Priyanka', 'Debashish', 'Seedance', 'Nano Banana', 'Higgsfield'];
const unsafe = [];
...
check('rejects an all-common phrase', phon.isSafeVariant('no way', 'Navya'), false);
check('accepts a mixed phrase', phon.isSafeVariant('sub rajit', 'Subhrajit'), true);

// --- end to end through the dictionary ----------------------------------

const taught = dict.learn([], 'I met sub trees today', 'I met Subhrajit today', []);
check('one correction is taught', taught.learned, [{ from: 'sub trees', to: 'Subhrajit' }]);
check('phrases hold only what the user taught', taught.phrases, [{ from: 'sub trees', to: 'Subhrajit' }]);
check('variants were generated', taught.variants.length > 0, true);
check('every variant points at the canonical', taught.variants.every((v) => v.to === 'Subhrajit'), true);
