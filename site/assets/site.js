...
/* ---------- the orb: the app's own flow-bar sphere, driven by the demo state ---------- */
(function () {
  'use strict';
  if (!window.VoxdenEnergyOrb) return;
  var canvases = document.querySelectorAll('.demo-orb');
  if (!canvases.length) return;
  var reduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  var orbs = Array.prototype.map.call(canvases, function (c) {
    return { renderer: window.VoxdenEnergyOrb.create(c), bar: c.closest('.demo-bar'), phase: Math.random() * 6 };
  });
  var time = 1.7, last = 0, visible = true, frame = 0;
  function paint(dt) {
    orbs.forEach(function (o) {
      var state = o.bar ? o.bar.getAttribute('data-state') : 'idle';
      var recording = state === 'recording';
      var thinking = state === 'thinking';
      var voice = recording ? .34 + .18 * (.5 + .5 * Math.sin((time + o.phase) * 6.3)) + .08 * Math.abs(Math.sin((time + o.phase) * 17)) : .2;
      o.renderer.draw({ time: time + o.phase, energy: voice, hover: recording ? 1 : 0, processing: thinking, reducedMotion: reduced });
    });
  }
  function tick(stamp) {
    frame = 0;
    if (!visible || document.hidden) return;
    var dt = last ? Math.min(.05, Math.max(0, (stamp - last) / 1000)) : 0;
    last = stamp; time += dt;
    paint(dt);
    frame = requestAnimationFrame(tick);
  }
  function wake() { if (!frame && !reduced) { last = 0; frame = requestAnimationFrame(tick); } }
  if (reduced) { paint(0); return; }
  if ('IntersectionObserver' in window) {
    var seen = 0;
    var io = new IntersectionObserver(function (entries) {
      entries.forEach(function (e) { seen += e.isIntersecting ? 1 : -1; });
      visible = seen > 0; if (visible) wake();
    }, { threshold: 0.1 });
